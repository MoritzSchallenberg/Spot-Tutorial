...
Expected state: The `kortex_driver` hardware interface connects to the
...
/joint_states` that joint states are publishing.
...
Expected state: The `move_group` node is running, exposing the
...
Expected state: The arm moves according to the commanded trajectory or
twist.
...
Expected state: Gripper opens or closes.
